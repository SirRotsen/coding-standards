...
- `.claude/settings.json` wiring the two entry points: the `SessionStart` hook runs `$CLAUDE_PROJECT_DIR/.claude/bin/hq session-start` with `"timeout": 60`, and `permissions.allow` carries `Bash(.claude/bin/hq drop:*)`. Keep both exactly as written. The allow rule matches literal command text, so any other spelling of the path prompts and every other subcommand still prompts—and what it authorizes is code fetched from this repo, run without a prompt, in the process that holds `HQ_DROP_TOKEN`. The timeout covers a fetch chain of 10 seconds plus two document fetches at 10 each, then two untimed git calls; 20 was short of the fetches alone.
