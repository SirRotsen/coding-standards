...
- `.claude/settings.json` wiring the two entry points: the `SessionStart` hook runs `$CLAUDE_PROJECT_DIR/.claude/bin/hq session-start` with `"timeout": 60`, and `permissions.allow` carries `Bash(.claude/bin/hq drop:*)`. That allow rule matches the literal command text, so `hq drop` is allowed and every other subcommand still prompts. Know what it authorizes: code fetched from this repo, run without a prompt, in the process that holds `HQ_DROP_TOKEN`. That is why the ref is validated, the payload is checked before it is cached, and merging here is gated. The 60-second timeout is not decoration either: the bootstrap's own fetch allows 10 seconds, the payload it runs then fetches two documents at 10 more each, and after that come a `git ls-remote` and a `git fetch` that carry no timeout at all. 60 is headroom rather than a proof—the hook fails soft, so an overrun costs the standards for that session and nothing else—but 20, which the first conversion used, was under the fetches alone.
